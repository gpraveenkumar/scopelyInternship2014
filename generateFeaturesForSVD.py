...
# The following parameter is used a set the minimum number of facebook pages 
# a user has liked for him to considered as candidate while generating 
# recommnedations.
Cutoff = 25
...
def Map(L):
  ...
  f = open('data/unigram_aboveCutoff_1.txt')
  features = []
  for l in f:
      t = l.split(' ')
      #if int(t[1]) < Cutoff:
  ...
  for c,k in enumerate(L):
      ...
      if len(new_l) < 10:
          ...
          for i in range(1,len(new_l)):
              results.append(new_l[0]+' '+new_l[i]+'\n')
              
          if len(results) >= 1000000:
              f = open('data/featuresForSVD.csv','a')
              for i in results:
                  f.write(i)
              f.close()
              results = []
...
if __name__ == '__main__':
  ...
  f1 = open('data/unigram_aboveCutoff_1.txt','w')
  features = []
  for l in f:
      t = l.split(' ')
      if int(t[1]) < Cutoff:
  ...
  f = open('data/featuresForSVD.csv','w')
  f.close()  
